...
class SpinSystem:
    ...
    def branching_contrast(self, B=None, B_theta=None, B_phi=None, stated_levels=None):
        ...
        r_23_S_x = abs(gs_upper.conjugate().dot(bigSx.dot(es_lower.T)))**2
        r_13_S_x = abs(gs_lower.conjugate().dot(bigSx.dot(es_lower.T)))**2
    ...
    def transition_strength(self, initial, final):
        ...
        fx = abs(final.conjugate().dot(bigSx.dot(initial))) ** 2
        fy = abs(final.conjugate().dot(bigSy.dot(initial))) ** 2
        fz = abs(final.conjugate().dot(bigSz.dot(initial))) ** 2
        return fx, fy, fz
...
############################
    ...
    def compute_holeburning_spectrum(self, B, B_theta, B_phi, axis, gs_blobs, es_blobs):
        ...
        for gs_idx in gs_blob:
            ...
            for es_idx in es_blob:
                ...
                intensity_x = abs(es_vec.conjugate().dot(bigS_op_x.dot(gs_vec.T)))**2
                intensity_y = abs(es_vec.conjugate().dot(bigS_op_y.dot(gs_vec.T)))**2
                intensity_z = abs(es_vec.conjugate().dot(bigS_op_z.dot(gs_vec.T)))**2
        ...
        for gs_lower_idx in range(len(gs_blob)):
            ...
            for gs_upper_idx in range(gs_lower_idx, len(gs_blob)):
                ...
                ground_intensity_matrix[gs_upper_idx][gs_lower_idx] = abs(gs_upper_vec.conjugate().dot(bigS_op_axis.dot(gs_lower_vec.T)))**2
    ...
    @staticmethod
    def generate_spectrum_from_peaks(IN_FILE, FWHM, xmin, xmax):
        """ Given an in file with a list of transitions in the form 
        (gs_idx, es_idx, energy, intensity), creates the preliminary 
        holeburning spectrum by placing a Lorentzian with a fixed FWHM at each peak. """

        IN_FILE_SPLIT = IN_FILE.split("_")
        if IN_FILE_SPLIT[-1] != "peaks.txt":
            raise ValueError("The peaks input file should end with '_peaks.txt'. ")
        IN_FILE_SPLIT[-1] = "prelimspectrum.txt"
        OUT_FILE = "_".join(IN_FILE_SPLIT)

        # Read input values. Skip the 1st line which is just the column header.
        spectrum_data = np.loadtxt(IN_FILE, skiprows=1)
        filename_stripped = os.path.basename(IN_FILE)

        # x-axis grid to compute the Lorentzian values
        # We have at least 5000 points but also should have enough resolution
        # for the FWHM.
        ...
        np.savetxt(OUT_FILE, np.array(list(zip(E_grid, holeburn_spectrum))), header="Energy/GHz, Transmittance Spectrum", fmt=("%.5e", "%.4e"))

    @staticmethod
    def plot_holeburning_spectrum(IN_FILE, damp_factor, convolution, conv_width, linewidth, PLOT_ONLY):
        """ Takes in a the preprocessed spectrum file and then creates the final
        spectrum after damping by an overall factor, convolving it if the convolution
        option is True, then plots it on a graph. Also saves the final file. 
        The reason why we split it into 2 steps is because it takes a long time
        to go through a large number of transitions for complicated environments,
        while this step of damping and convolution is fast so we can sweep over
        these params easily. """

        if not PLOT_ONLY:
            IN_FILE_SPLIT = IN_FILE.split("_")
            if IN_FILE_SPLIT[-1] != "prelimspectrum.txt":
                raise ValueError("The peaks input file should end with '_prelimspectrum.txt'. ")
            IN_FILE_SPLIT[-1] = "spectrum.txt"
            OUT_FILE = "_".join(IN_FILE_SPLIT)

        # Read input values.
        data = np.loadtxt(IN_FILE, skiprows=1)
        E_grid = data[:, 0]
        holeburn_spectrum = data[:, 1]
        xmin, xmax = min(E_grid), max(E_grid)

        # Read the filename_stripped name to get the params to be used for the plot's labels
        filename_stripped = os.path.basename(IN_FILE)
        B, B_theta, B_phi = [float(x)  for x in filename_stripped.split("_")[1:4]]
        axis, label = filename_stripped.split("_")[4:6]
        ...
        if not PLOT_ONLY:
            ...
            # Leftover code for other windows/other smoothing methods
            # window = scipy.signal.gaussian(len(E_grid), 10) / 5
            # holeburn_spectrum = scipy.signal.savgol_filter(holeburn_spectrum,31, 3)
            
            np.savetxt(OUT_FILE, np.array(list(zip(E_grid, holeburn_spectrum))), header="Energy/GHz, Transmittance Spectrum", fmt=("%.4f", "%.4e"))
    ...
    def plot_experimental_spectrum(IN_FILE):
        ...
        plt.plot(xdata, ydata, c='k', linewidth=2)
...
# A.plot_holeburning_spectrum(IN_FILE="SpectrumData_0.661_0.33929_0_x_1V.txt", FWHM=0.000028, damp_factor=0.12, convolution=True, conv_width=0.0095, linewidth=2)
