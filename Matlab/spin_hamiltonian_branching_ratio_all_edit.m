...
ybyvo171_2 = yb171yvospinhamiltonian;
...
ybyvo171_2.find_transition_strengths_amplitude_v2(bvec,anglewrtc*pi/180,0)
...
heights171 = 0.2*ones(length(ybyvo171.transition_strengths_par(1,:)),1);
...
parmat2 = zeros(length(bvec),length(x));
parmat3 = zeros(length(bvec),length(x));
...
for it = 1:length(bvec)
    ...
    parmat2(it,:) = exp(-5*(lorentzian_multiple( x, ybyvo171_2.transitions(it,:), widths171, heights171)));
    parmat3(it,:) = exp(-5*(lorentzian_multiple( x, ybyvo171_2.transitions(it,:), widths171, ybyvo171_2.transition_strengths_perpy(it,:))));
...
% disp(ybyvo171.transition_strengths_par(end,:))
% disp(ybyvo171.transition_strengths_perpx(end,:))
% disp(ybyvo171.transition_strengths_perpy(end,:))

%==========================================================================
% Fig 1
figure('name','new: BperpC, EparC');
%surf(bvec*1000, x, perpmat','linestyle','none')
imagesc(bvec*1000, x, parmat2',[0 1])
%hold on
...
figure('name','new: BperpC, EparC');
%surf(bvec*1000, x, perpmat','linestyle','none')
imagesc(bvec*1000, x, parmat3',[0 1])
%hold on
%plot(bvec*1000,ybyvo.transitions)
set(gca,'YDir','normal')
c = bone;
...
% disp(ybyvo171.transition_strengths_par(2,:))
% figure();
% plot(x, parmat(end, :))
%==========================================================================
%%
...
it = 1;
mat171 = (lorentzian_multiple( x, ybyvo171.transitions(it,:), widths171, ybyvo171.transition_strengths_perpy(it,:)));
