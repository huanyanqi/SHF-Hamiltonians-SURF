...
class SpinSystem:
    ...
    def plot_transitions_strengths(self, B_range, B_theta, B_phi, transition_type, axis=None):
        ...
        if axis is None:
            plt.figure()
            plt.suptitle("Optical Transitions between energy levels")
            plt.subplot(131)
    ...
    def plot_superhyperfine(self, B_range, B_theta, B_phi):
        ...
        ax3.plot(B_range, [self.superhyperfine_levels('gs', b, B_theta, B_phi)[2] for b in B_range], c='C0', label="Ground state")
        ax3.plot(B_range, [self.superhyperfine_levels('es', b, B_theta, B_phi)[2] for b in B_range], c='C1', label="Excited state")
...
A = SpinSystem(
    ...
    nuclear_zeeman_rei * 0.49367 ,       # Nd even isotopes                  # REI Nuclear g coeff
    nuclear_zeeman_host * 0.2754968, # Yttrium-89                    # Host Nuclear g coeff
    1/2,                                 # Y-89                     # Host Nuclear spin
    superhyperfine * 10**-10 * np.array((-3.56, 0, -4.72))  # Displacement vector to host
    )
...
# A.plot_energies_brange(np.arange(0, 1.01, 0.01), 0, 0)
...
# A.plot_optical_transitions(np.arange(0, 0.301, 0.0005), np.pi/4, 0)

A.plot_transitions_strengths(np.arange(0,0.501,0.001), np.pi/4, 0, transition_type="optical", axis=None)
# A.plot_optical_transitions_strengths_fixed_B(0.10, np.pi/4, 0, axis=None)

# A.plot_superhyperfine(np.arange(0.0005, 0.1, 0.001), 0, 0)
